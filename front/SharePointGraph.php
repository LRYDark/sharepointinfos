...
class PluginSharepointinfosSharepoint extends CommonDBTM {
    ...
    /**
     * Fonction pour obtenir l'ID d'une liste SharePoint par son nom
     */
    public function getListId($siteId, $listName) {
        $accessToken = $this->getAccessToken();
        ...
        if ($httpStatus === 200 && isset($responseObj['value'])) {
            // Rechercher la liste par son nom (displayName)
            foreach ($responseObj['value'] as $list) {
                if (isset($list['displayName']) && $list['displayName'] === $listName) {
                    return $list['id'];
                }
            }
            throw new Exception("Liste '$listName' introuvable dans le site SharePoint.");
        } else {
            throw new Exception("Impossible de récupérer les listes : HTTP $httpStatus - " . $response);
        }
    }

    /**
     * Fonction pour interroger les éléments d'une liste SharePoint avec filtre sur le titre
     */
    public function queryListItemsByTitle($siteId, $listId, $title) {
        $accessToken = $this->getAccessToken();

        // Encoder le titre pour l'URL et construire le filtre OData
        $filter = "fields/Title eq '" . str_replace("'", "''", $title) . "'";
        $url = "https://graph.microsoft.com/v1.0/sites/$siteId/lists/$listId/items?\$expand=fields&\$filter=" . urlencode($filter);
        ...
        if ($httpStatus === 200 && isset($responseObj['value'])) {
            ...
            throw new Exception("Impossible d'interroger la liste : HTTP $httpStatus - " . $response);
        }
    }
